...
SettlementEngine::SettlementEngine(boost::asio::io_context& io,
    ...
    : io_(io), ledger_(ledger), interval_(intervalSeconds),
      postgresWriter_([]
        {
            const char* pg_uri = std::getenv("POSTGRES_URI");
            if (!pg_uri) {
                SPDLOG_WARN("❗ POSTGRES_URI not set. Falling back to default 'postgresql://localhost/testdb'");
                return std::string("postgresql://localhost/testdb");
            }
            return std::string(pg_uri);
        }()) {

...
MetricsServer::MetricsServer(std::shared_ptr<Ledger> ledger, int port)
    : ledger_(ledger), port_(port), server_(std::make_unique<httplib::Server>()) {}
...
void MetricsServer::stop() {
    if (!started_) return;  // ✅ Prevent join on unstarted thread

    if (server_) {
        server_->stop();  // 🔁 break from listen()
    }

    if (server_thread_.joinable()) {
        server_thread_.join();  // ✅ Only if it’s safe
    }

    started_ = false;  // Reset
}